        ...
        {isSubscribed ? (
          <motion.button
            ...
            onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
              ...
              onClick?.(e);
              setTimeout(() => {
                setIsSubscribed(subscribeStatus); // Revert after 2 seconds
              }, 2000);
            }}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <motion.button
            ref={ref}
            className={cn(
              "relative flex h-10 w-fit cursor-pointer items-center justify-center rounded-lg border bg-primary px-6 ",
              className,
            )}
            onClick={(e) => {
              setIsSubscribed(true);
